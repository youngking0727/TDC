...
val_loader_names = ['env_{}'.format(idx2train_env[i])
...
for step in range(start_step, n_steps):
    ...
    minibatches_device = [(d.float().to(device), t.float().to(device), y.float().to(device))
    ...
    if step % 5 == 0:
        ...
        print('Training at Step ' + str(step + 1) +
                        ' with loss ' + str(step_vals['loss']) +\
                        " and with training pcc " + str(step_vals['training_pcc'])) 
...
with open(args.output_dir + '/' + args.algorithm + '_' + str(args.seed) + '_res.pkl', 'wb') as f:
